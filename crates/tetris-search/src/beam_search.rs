    ...
    ScoreFn: Fn(&S) -> f32 + Clone,
{
    ...
    // Optional predicate that rejects child states before they enter the beam.
    // Return `true` to keep, `false` to discard.
    filter_fn: Option<fn(&S) -> bool>,
}
    ...
    ScoreFn: Fn(&S) -> f32 + Clone,
{
    ...
    /// Set a filter predicate. States where `f(state)` returns `false` are
    /// discarded before entering the beam.
    #[inline_conditioned(always)]
    pub fn with_filter(mut self, f: fn(&S) -> bool) -> Self {
        self.filter_fn = Some(f);
        self
    }

    #[inline_conditioned(always)]
    pub fn set_filter(&mut self, f: fn(&S) -> bool) {
        self.filter_fn = Some(f);
    }
    ...
    ScoreFn: Fn(&S) -> f32 + Clone + Send + Sync,
{
    ...
    /// Set a filter predicate on all inner beam searches.
    pub fn with_filter(mut self, f: fn(&S) -> bool) -> Self {
