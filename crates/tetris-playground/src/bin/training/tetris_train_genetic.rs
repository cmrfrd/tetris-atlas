...
impl EvalCoefficients {
    ...
    pub fn mutate<R: rand::Rng>(&mut self, rng: &mut R, mutation_rate: f32) {
        if rng.random::<f32>() < mutation_rate {
            self.lines_cleared += rng.random_range(-0.05..0.05);
            self.lines_cleared = self.lines_cleared.clamp(MIN, MAX);
        }
        if rng.random::<f32>() < mutation_rate {
            self.height += rng.random_range(-0.05..0.05);
            self.height = self.height.clamp(MIN, MAX);
        }
        if rng.random::<f32>() < mutation_rate {
            self.bumpiness += rng.random_range(-0.05..0.05);
            self.bumpiness = self.bumpiness.clamp(MIN, MAX);
        }
        if rng.random::<f32>() < mutation_rate {
            self.weighted_holes += rng.random_range(-0.05..0.05);
            self.weighted_holes = self.weighted_holes.clamp(MIN, MAX);
        }
        if rng.random::<f32>() < mutation_rate {
            self.cell_count += rng.random_range(-0.05..0.05);
            self.cell_count = self.cell_count.clamp(MIN, MAX);
        }
    }
    ...
    pub fn crossover<R: rand::Rng>(parent1: &Self, parent2: &Self, rng: &mut R) -> Self {
        Self {
            ...
            cell_count: if rng.random_bool(0.5) {
                parent1.cell_count
            } else {
                parent2.cell_count
            },
...
impl BeamTetrisStateGenetic {
    ...
    pub fn score(&self) -> f32 {
        ...
        let cell_count = self.game.board.count() as f32;
        ...
        self.coefficients.lines_cleared * lines
            ...
            + self.coefficients.cell_count * cell_count
    }
}
...
impl<const N: usize, const BEAM_WIDTH: usize, const MAX_DEPTH: usize, const MAX_MOVES: usize>
    Iterator for TetrisGameIterGenetic<N, BEAM_WIDTH, MAX_DEPTH, MAX_MOVES>
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let action_votes: HashMap<TetrisPiecePlacement, (usize, f32)> = self
            .searches
            .par_iter_mut()
            .enumerate()
            .filter_map(|(i, search)| {
...
fn evolve_coefficients() {
    ...
    // --- Tunables ---
    const N: usize = 8; // Number of parallel searches for multi-beam voting
    ...
    writeln!(
        writer,
        "generation,best_fitness,avg_fitness,pieces_placed,avg_height,lines_cleared,height,bumpiness,weighted_holes,cell_count"
    )
