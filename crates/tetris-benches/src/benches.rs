...
// ============================================================================
// Heap Benchmarks: FixedBinMinHeap
// ============================================================================
...
tetris_bench! {
    ...
    pub fn bench_heap_push_if_better(c: &mut Criterion) {
        ...
        group.bench_function("push_if_better", |b| {
            let mut heap = FixedBinMinHeap::<i32, HEAP_SIZE>::new();
            b.iter(|| {
                heap.clear();
                for &v in candidates.iter() {
                    heap.push_if_better_min_heap(black_box(v));
                }
...
tetris_bench! {
    ...
    pub fn bench_heap_size_scaling(c: &mut Criterion) {
        ...
        for size in [16, 64, 128] {
            ...
            match size {
                16 => {
                    group.bench_function("size_16", |b| {
                        let mut heap = FixedBinMinHeap::<i32, 16>::new();
                        b.iter(|| {
                            heap.clear();
                            for &v in &candidates {
                                heap.push_if_better_min_heap(black_box(v));
                ...
                64 => {
                    group.bench_function("size_64", |b| {
                        let mut heap = FixedBinMinHeap::<i32, 64>::new();
                        b.iter(|| {
                            heap.clear();
                            for &v in &candidates {
                                heap.push_if_better_min_heap(black_box(v));
                ...
                128 => {
                    group.bench_function("size_128", |b| {
                        let mut heap = FixedBinMinHeap::<i32, 128>::new();
                        b.iter(|| {
                            heap.clear();
                            for &v in &candidates {
                                heap.push_if_better_min_heap(black_box(v));
...
tetris_bench! {
    pub fn bench_bits_to_byte(c: &mut Criterion) {
        ...
        c.bench_with_input(
            ...
            &NUM_ELEMS,
            |b, _| b.iter(|| bits.iter().map(|bits| tetris_utils::bits_to_byte(bits)).collect::<Vec<_>>()),
        );
    }
}
...
tetris_bench! {
    pub fn bench_byte_to_bits(c: &mut Criterion) {
        ...
        c.bench_with_input(
            ...
            &NUM_ELEMS,
            |b, _| b.iter(|| bytes.iter().map(|b| tetris_utils::byte_to_bits(*b)).collect::<Vec<_>>()),
        );
    }
}
