...
/// Standard MLP configuration with multiple hidden layers
...
/// Each hidden layer has SiLU activation and optional dropout
#[derive(Debug, Clone)]
pub struct Mlp {
    layers: Vec<Linear>,
...
impl Mlp {
    pub fn init(vb: &VarBuilder, cfg: &MlpConfig) -> Result<Self> {
        ...
        let mut layer_sizes = vec![cfg.input_size];
...
/// Attention-style MLP configuration (gated/SwiGLU architecture)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AttnMlpConfig {
...
/// Attention-style MLP with gated activation (SwiGLU)
/// Used in transformer blocks: silu(gate_proj(x)) * up_proj(x) -> down_proj
#[derive(Debug, Clone)]
pub struct AttnMlp {
    c_fc1: Linear,
    c_fc2: Linear,
...
impl AttnMlp {
    pub fn init(vb: &VarBuilder, cfg: &AttnMlpConfig) -> Result<Self> {
        let span = tracing::span!(tracing::Level::TRACE, "attn_mlp");
...
impl TransformerBlock {
    pub fn init(vb: &VarBuilder, cfg: &TransformerBlockConfig) -> Result<TransformerBlock> {
        ...
        let mlp = AttnMlp::init(&vb.pp("mlp"), &cfg.mlp_config)?;
        Ok(TransformerBlock {
            dyn_tanh_1,
            attn,
            dyn_tanh_2,
...
impl ConvEncoder {
    pub fn init(vb: &VarBuilder, cfg: &ConvEncoderConfig) -> Result<Self> {
        ...
        let mlp = AttnMlp::init(&vb.pp("mlp"), &cfg.mlp)?;
        Ok(Self {
            blocks,
            flatten_dim,
            mlp,
