use clap::Parser;
use std::str::FromStr;
use tetris_ml::{Conv2dConfig, ConvBlockSpec, set_global_threadpool};
...
use tetris_ml::data::TetrisDatasetGenerator;
use tetris_ml::grad_accum::GradientAccumulator;
use tetris_ml::modules::{
    AttnMlp, AttnMlpConfig, ConvEncoder, ConvEncoderConfig, FiLM, FiLMConfig,
};
use tetris_tensor::ops::create_orientation_mask;
use tetris_tensor::ops::get_l2_norm;
...
pub fn train_goal_policy(
    ...
    let vb = VarBuilder::from_varmap(&model_varmap, tetris_tensor::fdtype(), &device);
...
fn main() {
    info!("Starting tetris world goal policy training");
