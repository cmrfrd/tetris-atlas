...
pub fn train_population(
    ...
    let mut game_set = if USE_SHARED_GAME_SEEDS {
        tetris_game::TetrisGameSet::new_with_same_seed(eval_game_seed(shared_seed_base, 0), POPULATION_SIZE)
    } else {
        tetris_game::TetrisGameSet::new_with_seed(shared_seed_base, POPULATION_SIZE)
    };
...
fn main() {
    info!("Starting tetris evolution training");
    ...
    train_population(run_name, logdir, checkpoint_dir).unwrap();
}
