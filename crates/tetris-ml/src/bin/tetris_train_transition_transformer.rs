...
use tetris_ml::optim::{AdamW, ParamsAdamW};
...
use tetris_ml::{
    ...
    modules::{
        AttnMlp, AttnMlpConfig, CausalSelfAttentionConfig, DynamicTanhConfig, FiLM, FiLMConfig,
        Mlp, MlpConfig, TransformerBlockConfig, TransformerBody, TransformerBodyConfig,
    },
};
use tetris_tensor::ops::{binary_cross_entropy_with_logits_stable, get_l2_norm};
...
pub struct TetrisGameTransitionModelConfig {
    ...
    pub block_mlp_config: AttnMlpConfig,        // hidden=d_model, inter, output=d_model
    pub cell_head_config: AttnMlpConfig,        // hidden=d_model, inter, output=1
...
impl TetrisGameTransitionModel {
    pub fn init(
        ...
        let cell_head = AttnMlp::init(&vb.pp("cell_head"), &cfg.cell_head_config)?;
...
pub fn train_game_transition_model(
    ...
    let model_vs = VarBuilder::from_varmap(&model_varmap, tetris_tensor::fdtype(), &device);
    ...
    let model_cfg = TetrisGameTransitionModelConfig {
        ...
        cell_head_config: AttnMlpConfig {
            hidden_size: model_dim,
            intermediate_size: 2 * model_dim,
            output_size: 1,
            dropout: None,
...
fn main() {
    info!("Starting tetris transition transformer training");
