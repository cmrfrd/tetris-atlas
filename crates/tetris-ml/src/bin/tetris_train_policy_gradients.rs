...
use tetris_ml::modules::{
    AttnMlpConfig, CausalSelfAttentionConfig, DynamicTanhConfig, Mlp, MlpConfig,
...
pub fn train_tetris_policy_gradients(
    ...
    for iteration in 0..NUM_ITERATIONS {
        ...
        while trajectories.len() < NUM_TRAJECTORIES_PER_BATCH {
            ...
            let games_ptr = SendPtr(&mut active_games as *mut tetris_game::TetrisGameSet);
...
fn main() {
    info!("Starting tetris policy gradients training");
    ...
    train_tetris_policy_gradients(run_name, logdir, checkpoint_dir).unwrap();
}
