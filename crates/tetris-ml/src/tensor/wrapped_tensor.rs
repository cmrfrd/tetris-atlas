...
macro_rules! impl_wrapped_tensor {
    ($ty:ty, dtype = $dtype:expr, shape_spec = ( $($spec:expr),* $(,)? )) => {
        impl $crate::tensor::wrapped_tensor::sealed::Sealed for $ty {}
        impl $crate::tensor::wrapped_tensor::WrappedTensor for $ty {
            const RANK: usize = $crate::__count_tuple_elements!( $($spec),* );

            type ShapeSpec = $crate::__tensors_type_tuple_by_specs!( $crate::tensor::wrapped_tensor::ShapeDim ; $($spec),* );
            ...
            fn shape_ok(tensor: &candle_core::Tensor) -> ::anyhow::Result<()> {
                let expected: &[$crate::tensor::wrapped_tensor::ShapeDim] = &[$($spec),*];
                ...
                for (axis, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
                    match *e {
                        $crate::tensor::wrapped_tensor::ShapeDim::Any => {},
                        $crate::tensor::wrapped_tensor::ShapeDim::Dim(n) => {
                            if *a != n {
                                return Err(::anyhow::anyhow!(
        ...
        impl std::convert::TryFrom<candle_core::Tensor> for $ty {
            ...
            fn try_from(tensor: candle_core::Tensor) -> ::anyhow::Result<Self> {
                let expected = <$ty as $crate::tensor::wrapped_tensor::WrappedTensor>::expected_dtype();
                ...
                // Shape check
                <$ty as $crate::tensor::wrapped_tensor::WrappedTensor>::shape_ok(&tensor)?;
