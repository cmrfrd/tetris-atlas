...
| **`cycle_drop_height_cap`**, `legal_cycle_release_window`, `cycle_release_cap`, `wordDropSum`, `wordLift_ge_drops`, `wordLift_le_drops`, `gravityInt_le_rowMoment`, `shapeRowMoment_le_six`, `shapeRowMoment_O`, `shapeRowMoment_eq_six_iff` | Reading the potential balance: a survivor never falls further than it stood; shape moments are ≤ 6 with only the vertical I reaching six; and around a cycle 4·(total landing height) ≤ what the clears release ≤ 4·(landing height) + 6 per move — a 210-wide window for a 35-cycle |
| **`cycle_potential_balance`**, `rowMoment`, `shapeRowMoment`, `gravityInt`, `clearedRowSum`, `rowMoment_place`, `rowMoment_clearLines`, `rowMoment_word` | THE POTENTIAL BALANCE: summing cells' row indices gives a potential that drops raise by 4·(landing height) + shape moment and clears lower by 10·(height of each removed row) + the survivors' total fall. Around a cycle these balance EXACTLY (not modulo anything) — the first cycle law that sees how HIGH the play happens |
| **`legal_cycle_has_flat_T`**, `shapeRowCharge_eq_one_iff`, `shapeRowCharge_of_not_flat_T`, `wordPieceCharge` (+`_eq_count`), `wordRowCharge_of_no_flat_T` | EVERY LEGAL 35-CYCLE LAYS A T FLAT. The row-moment table is clean — I/O/S/Z even in every rotation, L/J odd in every rotation, T odd exactly in its two FLAT rotations — so L and J contribute five apiece (even) and the whole odd row moment must come from the T's: an odd number of the cycle's five T's are laid flat, in particular at least one |
| **`cycle_rotation_census`**, `rowCharge`, `shapeRowCharge`, `rowCharge_place`, `rowCharge_clearLines`, `rowCharge_word`, `cycle_rowCharge_law`, `legal_cycle_rowCharge_odd` | THE ROW CHARGE, the checkerboard's other half: a cleared row contributes 10·t ≡ 0, so a clear changes it by the GRAVITY WORK ALONE. Reading the gravity work both ways pins the shapes' total ROW MOMENT to the T-count plus cleared rows — which ROTATIONS a cycle plays is no longer free, and on a legal 35-cycle the row moment is ODD |
| **`legal_cycle_has_odd_or_split_clear`**, `gravityWork_of_contiguous_even`, `gravityWork_tetris`, `gravityWork_double`, `exists_odd_or_split_clear` | EVEN CONTIGUOUS CLEARS ARE INVISIBLE TO THE CHARGE (every survivor falls by zero or by the whole even block), so doubles and tetrises do no gravity work — and A LEGAL CYCLE CANNOT LIVE ON THEM: some move must clear a single, a triple, or a split block straddling an unfilled row |
| **`legal_cycle_has_partial_clear`**, `no_legal_perfect_clear_cycle`, `gravityWork_of_no_clears`, `gravityWork_of_perfect`, `partial_of_gravityWork`, `exists_partial_clear` | Only a PARTIAL clear can do gravity work — a dry move has nothing falling, a perfect clear has nothing left to fall — so a legal 35-cycle must somewhere remove a row while leaving a cell standing. In particular NO LEGAL CYCLE IS A PERFECT-CLEAR LOOP: the T-parity that forbids five bags tiling a rectangle survives the passage to loops WITH clears |
| **`legal_cycle_gravity_odd`**, `exists_loaded_clear`, `cycle_charge_law`, `charge_word`, `wordTCharge`, `wordGravity`, `exists_cell_above_clear` | THE CYCLE CHARGE EQUATION and its payoff: around a loop the gravity work equals the T-count plus the cleared rows (mod 2). On a legal 35-cycle five T's (odd) against fourteen rows (even) FORCE the gravity work to be one — so SOME CLEAR CARRIES A LOAD: not every clear can be a clean sweep off the top; at least one happens with surviving cells stacked above it. Clear geometry derived from the piece census alone |
| **`charge_clearLines`**, `charge_applyStep`, `gravityWork`, `charge_cleared_cells`, `cleared_fiber_eq`, `cast_sub_two` | THE CHARGE LAW ACROSS A CLEAR: the checkerboard charge (only T is charged) is normally abandoned at a clear because gravity RECOLOURS the board — but the recolouring is exactly the cells with an odd number of cleared rows beneath them. Naming that *gravity work* turns a destroyed invariant into an exact law: Δcharge = (rows removed) + (gravity work). The first tool coupling piece TYPE to WHERE the clears happen |
| **`cycle_through_empty_clears_bottom`**, `lowCells`, `lowCells_clearLines`, `dropOffset_emptyset`, `bottomCells_place_empty_nonempty` | THE FROZEN FOUNDATION at every depth (a clear sparing rows ≤ r fixes the whole band), plus: the opening piece always lands on the floor, so ANY CYCLE THROUGH THE EMPTY BOARD MUST CLEAR ITS BOTTOM ROW — an unconditional structural fact about every ∅-cycle |
| **`clearLines_band_below`**, `millStackWith`, `millBagWith`, `well_I_sweeps_to_floorE`, `millStackWith_floorE_card` | THE MILL CARRIES A PAYLOAD: a full k-row band at the bottom clears to exactly the payload above it, lowered by k. So the mill is not a special ENDING but a reusable MOVE - `millBagWith` returns any payload, and `millBag` is the empty-payload case. `well_I_sweeps_to_floorE` is the instance a fourth bag ending in a tetris would need |
| **`clear_free_word_charge`**, `clear_free_charge_change`, `clear_free_bag_flips_charge`, `millFloor_charge`, `applyStep_eq_place_of_no_clear`, `charge_millStack`, `charge_floorE` | THE PARITY TEST FOR TILINGS - the first constraint from GEOMETRY rather than counting. T is the only unbalanced tetromino, so along a clear-free stretch the checkerboard charge changes by the T-count and nothing else. A bag deals one T, so A CLEAR-FREE BAG ALWAYS FLIPS THE CHARGE. Turns "can six pieces build this floor into the mill stack?" into ONE kernel evaluation, before any placement is chosen |
| **`CycleWitness.bag_profile`**, `bag_boundary_mass`, `last_two_bags_clear_six`, `last_three_bags_clear_nine`, `last_four_bags_clear_twelve` | THE BAG PROFILE - the whole story at bag resolution: cumulative clears after 1/2/3/4 bags are capped at 2/5/8/11 and total 14, so the last bag takes >= 3, the last two >= 6, the last three >= 9, the last four >= 12. `bag_boundary_mass` ties the profile to standing inventory (10*P_j + mass = 28j), and the per-bag cap turns out to BE the prefix ceiling - nothing further is hidden |
| **`bag_ledger`**, `bag_clears_le`, `bag_on_empty_le_two`, `bag_tetris_needs_twelve`, `bag_five_needs_twentytwo`, `Segment.clears_le`, `clearSeq_append`, `millBagE_extremal` | WHAT ONE BAG CAN TAKE: ten times a bag's harvest, plus the mass it leaves, is the mass it found plus the 28 cells it laid - so a bag never takes more than (mass+28)/10. Empty board: 2. A TETRIS IN ONE BAG NEEDS 12 CELLS STANDING, exactly what the mill floor carries, so `millBagE` is EXTREMAL. Five rows needs 22 |
| **`clearSeq_bag_ceiling`**, `first_bag_le_two`, `two_bags_le_five`, `three_bags_le_eight`, `four_bags_le_eleven`, `clearSeq_entry_bound`, `no_clear_before_third`, `no_double_before_fifth`, `no_triple_before_eighth`, `no_tetris_before_tenth`, `CycleWitness.last_bag_clears_three`, `final_mass`, `final_mass_le_thirty_six`, `final_mass_cases` | READING THE SCHEDULE. Bag by bag: at most 2 / 5 / 8 / 11 rows after 1 / 2 / 3 / 4 bags, so the LAST bag takes >= 3. Move by move: a c-row clear needs 5c <= 2k+2 - single at move 3, double at 5, triple at 8, tetris at 10. And the ending is one of FOUR kinds: the mass before the last placement is 6, 16, 26 or 36, so at most a mill goes into the final move |
| **`clearSeq`**, `clearSeq_sum`, `clearSeq_take`, `clearSeq_entries_le_four`, `clearSeq_prefix_mass`, `clearSeq_ballot`, `clearSeq_opening_dry`, `CycleWitness.clearSeq_tail_ge`, `last_move_clears`, `last_five_clear_two`, `last_ten_clear_four`, `segABC_clearSeq`, `millBagE_clearSeq` | THE CLEAR SCHEDULE as a first-class object: a word over {0,..,4}, one entry per move. Its partial sums ARE the mass trajectory (10*S_k + mass_k = 4k), so from empty it obeys a BALLOT CEILING 5*S_k <= 2k; a five-bag loop must land on 14 = 2*35/5 EXACTLY, flush against a ceiling it may never cross. Hence the opening is dry and the tail owes 14 - 2k/5 |
| **`floorE`**, `sixE`, `sixE_builds`, `sixE_no_clears`, `sixE_holeFree`, `millBagE`, `millBagE_clears`, `loop_closed_by_four_bags` | THE LAST BAG IS BUILT: twelve cells in a terrace across the left six columns, six pieces (every piece but the I) building them into the mill stack hole-free with NO premature clear, then the I sweeping it to the EMPTY BOARD. Seven placements, one of each piece. Four bags from empty to this floor would close the loop and settle M2 |
| **`millStack`**, `well_I_sweeps`, `millBag`, `mill_loop_survives`, `millStack_holeFree` | THE FINISH AS A TILING PROBLEM: nine columns four deep with the tenth open, swept by a vertical I. `millBag` turns any six placements carrying a floor to that stack into a finished bag ending on the empty board - no game theory left, just a rectangle-tiling question |
| **`forall_isFull_of_clearLines_empty`**, `CycleWitness.final_board_swept`, `every_cell_swept`, `tail_clears`, `mill_finish`, `banked_finish` | THE LAST MOVE SWEEPS THE BOARD BARE: for a witness anchored at the empty board every cell standing when the last piece lands - its own four included - must lie in a row that landing completes. Nothing may be left over. Three bags clearing six rows force TWO TETRISES to finish: the classical mill, forced rather than chosen |
| **`dropOffset_congr`**, `dropped_congr`, `dropped_row_ge_colHeight`, `buried_stays_buried`, `HoleFree`, `segABC_trace_holeFree` | THE SKYLINE IS EVERYTHING and A BURIED GAP IS FOREVER: where a piece lands depends on column heights alone, and every landing sits at or above the column height - so a cell with anything above it can never be filled by any placement, only disposed of by clearing its row. The formal reason hole-freeness binds a construction |
| **`bagC`**, `segC`, `segABC`, `segABC_clears`, `segABC_hole_free`, `shapeUp_flat_iff`, `piece_eq_I_of_flat`, `CycleWitness.flat_le_bags`, `segABC_remaining_clears` | A THIRD BAG (7 rows cleared in 21 placements from empty, still hole-free) plus THE FLAT-PIECE LEMMA: of all 28 piece-rotation pairs exactly one lies inside a single row - the horizontal I - so a period of m bags can close at most m one-tall gaps. Rules out most tempting endings |
| **`oneBag`**, `bagA`, `bagB`, `segA`, `segB`, `segAB`, `segAB_clears`, `segAB_dst_card`, `segAB_hole_free`, `segAB_remaining_clears` | THE FIRST CONCRETE LINKS: two hand-designed bags from the EMPTY BOARD, kernel-verified - in-bounds, in-field, HOLE-FREE at every step, clearing 1 then 4 rows and leaving six cells standing. `oneBag` is the reusable design tool. Nine of the fourteen owed rows remain, across three more bags |
| **`Segment`**, `Segment.comp`, `Segment.id`, `Segment.toWitnessFast`, `Segment.survives`, `Segment.five_chain_survives`, `mem_wordTrace_append`, `getD_append_left`/`_right` | SEGMENTS COMPOSE: a stretch of legal play (whole bags, board to board, in field) joins to any stretch starting where it ended, and a stretch ending where it began IS a certificate. Construction becomes incremental - design one bag, check it, chain it - instead of one thirty-five move guess |
| **`wordTrace`**, `foldl_take_mem_wordTrace`, `safe_of_trace_inField`, `WitnessConditionsFast`, `witnessConditions_of_fast`, `CycleWitness.ofFast`, `survives_of_witnessConditionsFast`, `ten_I_trace_inField` | SAFETY IN ONE PASS: the running trace lists the boards a word visits, each built from the last, and every prefix fold is in it - so the quadratic per-index safety check collapses to a single linear sweep. This is the form in which a concrete witness should be presented |
| **`CycleWitness.round_seven_dvd`**, `not_seven_copies_of_ten`, `not_repetition_of_partial_bag`, `length_eq_sum_count`, `count_flatten_replicate` | A WITNESS IS NEVER ONE ROUND REPEATED unless that round is a whole number of bags: repetition multiplies every piece count, so equal counts force 7 divides the round length. Seven copies of a ten-piece perfect-clear round is bag-ILLEGAL - the rounds of any such construction must genuinely differ |
| **`WitnessConditions`**, `witnessConditions_decidable`, `boardWF_decidable`, `CycleWitness.ofConditions`, `survives_of_witnessConditions`, `safe_of_inField`, `ten_I_cycle_empty_by_kernel`, `five_O_cycle_empty_by_kernel` | WITNESS-HOOD IS A FINITE CHECK: every field quantifies over a finite domain, closure and safety included, so a candidate can be certified by kernel evaluation alone — and the kernel demonstrably executes a ten-move loop with a tetris in it. Producing a witness is a search for the right WORD, not for the right proof |
| **`CycleWitness.perfect_clear_bags`**, `.perfect_clear_length_ge_seventy`, `.no_perfect_clear_five_bags`, `.clears_eq`, `natCast_two_eq_zero_iff` | A PERFECT-CLEAR LOOP NEEDS TEN BAGS: if every harvest sweeps the board bare the gravity work vanishes, the parity equation collapses to "bags/5 is even", and the period cannot be thirty-five — SEVENTY is the first length allowed. The T-parity obstruction, previously confined to clear-free rectangle tilings, now reaches loops that actually clear |
| **`CycleWitness`** (+`.survives`, `.closedCycle`, `.five_dvd_bags`, `.bags_ge_five`, `.piece_census`, `.clear_census`), `isBagStream_of_blocks_gen` | THE WITNESS AT ANY NUMBER OF BAGS: the certificate now names its bag count, the five-block test generalizes to an m-block test, and the theory itself supplies the constraint — EVERY witness runs a multiple of five bags, so one, two, three or four bags is impossible |
| **`safe_word_closedCycle_gen`**, `safe_word_survives_gen`, `wordPolicy_spec`, `safe_word_state_cycle`, `mem_wordOrbit_iff_gen`, `wordOrbit_length` | THE CERTIFIED CLOSED CYCLE AT ANY PERIOD: the construction needed neither the length 35 nor the orbit's distinctness — the table policy returns SOME index carrying the state, and that is all the four `ClosedCycle` fields require. Frees the target to length 70, which the T-parity says a perfect-clear loop must have |
| **`cycle_floor_tax`**, `floorGaps`, `debt_ge_floorGaps_of_full`, `floorGaps_nonempty`, `cycle_floorGaps_frozen` | THE FLOOR TAX (generalizing the empty-floor result to any gap): with any row complete every floor gap is a hole, so the debt is at least the gap count — and a cycle sparing its bottom row freezes those gaps, so since a cycling board is clear-free its floor always has a gap and its debt never falls to zero at a clearing moment |
| **`cycle_no_emptying_all_occupied`**, `occupiedCols`, `occupiedCols_subset_place`, `occupiedCols_eq_range_of_full`, `cycle_occupiedCols_frozen_split`, `wordColEmptied` | WHICH COLUMNS ARE STANDING: placement never empties a column, so a cycle that never empties one keeps the same standing set throughout — and since a completed row occupies all ten at once, such a cycle stands on all ten columns from the very start |
| **`consecutive_tetris_needs_seventytwo`**, `consecutive_clears_mass`, `count_of_dry_move`, `clears_of_count_le`, `count_after_clear` | BACK-TO-BACK HARVESTS: a tetris leaves the board 36 lighter and needs 36 to fire, so two in a row demand 72 cells standing beforehand — two fifths of everything a clear-free board can hold; generally consecutive k- and k′-clears need 10k + 10k′ − 8 |
| **`count_ge_of_clear`**, `count_ge_of_tetris`, `legal_cycle_dry_prefix_mass`, `legal_cycle_heavy_base_short_dry`, `sum_rowCount_le_count` | THE SAWTOOTH: a k-clear cannot happen unless 10k − 4 cells already stand (36 for a tetris), while between clears the board grows four a move and may never pass 180 — so a long dry opening forces a light base board and a heavy base cannot open with many dry moves |
| **`cycle_column_burial_conservation`**, `colBuried`, `colUnburied`, `colHoles_le_colHoles_place`, `colHoles_clearLines_le_std`, `colHoles_word`, `cycle_column_flush_iff` | BURIAL CONSERVATION, COLUMN BY COLUMN: each of the ten columns buries exactly as many holes as it frees around a cycle — ten equations where the global law gave one, and a column that never frees a hole never digs one |
| **`cleared_row_prior_six`**, `cleared_rows_prior_inventory_board`, `tetris_prior_thirtysix`, `rowCount_of_mem_fullRows`, `rowCount_place` | WHAT A CLEAR MUST ALREADY OWN: a piece brings four cells and a row needs ten, so a cleared row held ≥ 6 beforehand and a k-clear held ≥ 10k − 4 across its rows — thirty-six for a tetris. Harvests are collected, not created |
| **`fullRows_subset_span`**, `cleared_row_ge_dropOffset`, `clearedRowSum_ge_mul`, `clearedRowSum_le_mul` | A CLEAR HAPPENS WHERE THE PIECE LANDED: on a clear-free board every completed row owes its last cell to the piece just dropped, which spans only four rows from its landing height — so no clear happens below the landing height, and a harvest's total height is pinned between k·D and k·(D+3) |
| **`legal_cycle_tetris_le_three`**, `four_mul_tetrises_le_clears`, `legal_cycle_tetrises_lt_clearMoves`, `legal_cycle_small_clears_ge_two` | ONLY THREE TETRISES FIT: the clear budget beats the bag supply — fourteen rows at four apiece leaves room for three, not five — and since fourteen is not a multiple of four, the clearing moves cannot all be tetrises: every M2 witness contains a single, a double or a triple |
| **`LegalCycleWitness`** (+`.survives`, `.closedCycle`, `.profile`, `.T_split`, `.isBagStream`) | THE CERTIFICATE AS ONE OBJECT: base board, 35-word, well-formedness, length, validity, the five-block test, closure and safety. Four fields are finite decidable checks; the two that carry the mathematics are that the word CLOSES and never tops out. Inhabiting it settles M2, and every inhabitant automatically satisfies the nine-clause profile and the T-split |
| **`isBagStream_of_blocks`**, `sampleWord` (+`_length`, `_valid`, `_isBagStream`, `_census`, `_has_flat_T`) | BAG-LEGALITY IS A FINITE CHECK: the repeated stream's block j is the word's own block j mod 5, so checking a 35-word's five blocks settles every block of the stream — `IsBagStream` becomes decidable, demonstrated end-to-end on a concrete valid 35-word whose census (five of every piece) the kernel verifies |
| **`legal_cycle_column_quad_le_three`**, `legal_cycle_column_triple_le_four`, `legal_cycle_column_no_quad_feeds_five`, `mul_bigMoves_le_colProfile`, `colProfile_le_mul_feeds` | HOW A COLUMN'S FOURTEEN CELLS ARRIVE: the fixed budget limits delivery coarseness — at most three four-cell drops (vertical I's) per column, at most four three-cell drops, and a column that never takes a vertical I must be fed on at least five separate moves |
| **`legal_cycle_consecutive_boards_distinct`**, `legal_cycle_board_repeat_five_dvd`, `legal_cycle_board_ne_succ`, `prefix_mass_bound_180`, `foldl_clear_free_of_ne_nil` | REPEATED BOARDS ARE FIVE APART: the mass clock reads time mod five, so a loop revisiting a board does so only at multiples of five — five consecutive moments show five different boards, and the prefix band tightens from 200 to 180 |
| **`count_le_180`**, `rowCount_le_nine`, `count_eq_sum_rowCount`, `cycle_mass_le_180`, `cycle_base_mass_le_180` | THE CLEAR-FREE CEILING: a board with no complete row is missing a cell in every row, so it holds at most nine per row — 180, not the raw capacity 200. Every board a cycle visits is the image of a move, hence clear-free, so the ceiling for cyclic play is a tenth lower at EVERY moment |
| **`legal_cycle_uprightT_even`**, `colCharge`, `colCharge_eq`, `colCharge_place`, `colCharge_clearLines`, `cycle_colCharge_law`, `shapeColCharge_split3`, `wordUprightTCount`, `charge_eq_col_add_row` | ZERO, TWO OR FOUR: the column charge is the checkerboard charge plus the row charge (no new induction — char-two algebra on two proven laws), the gravity work cancels between the halves, and its shape table is the exact MIRROR (T odd only when UPRIGHT) — so a legal 35-cycle stands an even number of its five T's on end |
| **`legal_cycle_flatT_one_three_or_five`**, `legal_cycle_flatT_odd`, `wordFlatTCount`, `shapeRowCharge_split3`, `wordRowCharge_eq_counts`, `wordFlatTCount_le_T`, `cast_ite_one_zero`, `natCast_two_eq_one_iff` | ONE, THREE OR FIVE: the row charge splits exactly into L + J + flat-T indicators, the ten L's and J's cancel in pairs, so the whole parity debt lands on the flat T's — a legal 35-cycle lays an ODD number of its five T's flat, hence exactly one, three or five |
| **`cycle_empty_floor_debt`**, `colHoles_pos_of_floor_gap`, `debt_ge_ten_of_empty_floor`, `cycle_floor_stays_empty` | THE STANDING DEBT OF TEN: a cycle beginning with an empty bottom row that never completes it keeps row zero empty forever, so every occupied column stands over a gap — and at any clearing moment a full row occupies all ten columns at once, so the board carries ten holes. An empty floor is paid for continuously, not once |
| **`legal_cycle_profile`**, `wordDryMoves`, `wordClearMoves_add_dry`, `legal_cycle_dry_moves_ge` | THE M2 WITNESS PROFILE: nine forced clauses on any bag-legal 35-cycle — clear-free base; exactly 14 cells to each column; 14 rows cleared on 4–14 moves with ≥ 21 dry; ≤ 5 tetrises; the exact drop-column equation 630; exact potential balance; at least one PARTIAL clear; not buildable from doubles and tetrises; and at least one T laid flat |
| **`legal_safe_word_closedCycle`**, `legal_safe_word_survives`, `mem_wordOrbit_iff` | THE CERTIFIED CLOSED CYCLE: a bag-legal 35-word folding a well-formed board to itself and never topping out IS a `ClosedCycle` of the game — states, policy, validity, legal draws, safety and closure all discharged — and therefore yields `SurvivesForever`. The whole word theory lands in the library's canonical M2 structure |
| **`cycle_debt_frozen_split`**, `cycle_lowCells_frozen_split`, `wordBuried_append`, `wordUnburied_append`, `debt_word_ge`, `wordLowClear`, `lowCells_word_subset`, `wordLowClear_append` | The split technique applied twice more: a cycle that frees no holes carries the SAME debt on every board it visits (it cannot even temporarily dig and repay), and below the lowest row a cycle ever completes the foundation is identical at every single moment — a permanent substructure the play never touches |
| **`cycle_bottom_frozen_split`**, `legal_cycle_permanent_floor_gap`, `wordBottomClear_append`, `bottomCells_word_subset`, `cycle_bottom_gap_permanent` | THE BOTTOM ROW IS FROZEN THROUGHOUT (not just after the first move — proved by SPLITTING the word rather than indexing into it): a cycle that never fills its floor shows the same bottom row at every moment, so its floor gap is permanent while the column above it must still take delivery of fourteen cells |
| **`cycle_bottom_row_frozen`**, `bottomCells`, `bottomCells_clearLines`, `bottomCells_subset_place`, `bottomCells_card_word_ge`, `wordBottomClear` | THE BOTTOM-ROW DICHOTOMY: a clear that spares row zero leaves it exactly alone (a cell reaching row zero from above would need every row beneath it full), so a cycle either FILLS its bottom row at some point or never puts a single cell there — no middle course |
| **`cycle_prefix_monotone_const`**, `cycle_has_mass_drop`, `move_count_ne` | The obstruction sharpened to WORD-LOCAL monotonicity (no global hypothesis on Φ), which immediately catches: a move never leaves the mass unchanged (10k = 4 is unsolvable), so EVERY cycle must shed mass somewhere — the loop has to breathe out, it cannot drift up and land home |
| **`cycle_first_move_flush`**, `wordBuried_eq_zero_cons` | Flushness unfolded move by move: a cycle that recycles no holes lands its first piece perfectly flush — and with cycle rotation, every piece |
| **`prefix_clear_bound`**, `prefix_mass_bound`, `legal_cycle_prefix_clears` | THE CLEAR SCHEDULE: a constraint on every PREFIX, not just the total — you cannot clear faster than you deliver (10·clears ≤ base mass + 4·moves), nor fall more than a boardful behind while in-field |
| **`no_cycle_of_strict_increase`**, `cycle_monotone_const`, `foldl_monotone` | THE MONOTONE OBSTRUCTION: any quantity that never falls under a move is FROZEN on a cycle, so a single strict increase anywhere refutes the whole cycle — a general recipe for cycle non-existence proofs |
| **`legal_cycle_orbit_torus`**, `legal_cycle_coordinates_faithful`, `crt_five_seven` | THE ORBIT IS A 5×7 TORUS: the mass clock reads position mod 5, the bag clock mod 7, so the two clocks form a faithful COORDINATE SYSTEM — every (mass residue, bag size) pair occurs exactly once among the 35 states |
| **`cycle_burial_conservation`**, `cycle_flush_iff_no_recycling`, `wordBuried`, `wordUnburied`, `debt_word` | THE BURIAL CONSERVATION LAW: around any cycle the holes the placements bury are exactly the holes the clears set free — a cycle cannot bury on credit; and flush play ⟺ no hole recycling (no half-way cycle that only digs, none that only recovers) |
| **`colHeight_place_ge_add_colProfile`**, `sup_ge_add_card`, `debt_le_debt_place` | A column's skyline rises by at least what it receives (dropped cells occupy distinct rows at or above the old height) ⇒ HYPOTHESIS-FREE debt monotonicity under placement, improving the library route that needed in-field assumptions on every board |
| **`cycle_weighted_of_delivery`**, `wordWeightedProfile_eq_sum` | THE WEIGHTED FAMILY COLLAPSES: every weighted cycle law follows from the single per-column delivery law, so mining further weights yields nothing new — the boundary of the method, proven rather than guessed |
| **`legal_cycle_moment_630`**, `cycle_moment_exact`, `legal_cycle_quad_3990`, `cycle_quad_moment_exact`, `shapeQuad`, `weightedProfile_sq` | THE EXACT DROP-COLUMN EQUATIONS: read in drop coordinates the laws do NOT collapse — a legal 35-cycle satisfies ∑(4·col + shapeMoment) = 630 and ∑(4·col² + 2·col·shapeMoment + shapeQuad) = 3990, one linear and one quadratic exact Diophantine constraint on where the 35 pieces are dropped |
| **`cycle_weighted_law`**, `colWeight`, `weightedProfile`, `weightedProfile_eq_cell_sum`, `colWeight_applyStep`, `colWeight_word` | THE WEIGHTED-COLUMN LEDGER (engine): weight column j by w j and you get a conserved quantity for EVERY weight function — a cleared row surrenders one cell per column, so a clear always costs exactly Σw whatever the geometry. One equation per weight; an infinite family of cycle invariants |
| **`cycle_column_delivery`**, `legal_cycle_column_fourteen`, `legal_cycle_column_feed_moves`, `wordColProfile`, `colCount_word` | Weight = one column's indicator ⇒ EVERY COLUMN IS FED EXACTLY THE CLEAR COUNT: a legal 35-cycle delivers exactly fourteen cells to each of the ten columns (140 split ten ways, no remainder) and feeds each column on ≥ 4 of its moves |
| **`cycle_odd_parity`**, `oddW`, `oddProfile_col_free`, `legal_cycle_odd_delivery_even` | Weight = odd-column indicator ⇒ A PARITY LAW THAT SURVIVES CLEARS (unlike the checkerboard charge — gravity never moves a cell between columns): odd-column delivery ≡ cleared rows mod 2, and the per-piece odd charge is drop-position-independent mod 2 (4 − k ≡ k on a four-cell shape) |
| **`cycle_moment_mod_five`**, `sum_idW`, `shapeMoment`, `weightedProfile_id`, `cycle_drop_column_moment` | Weight = the column index ⇒ THE MOMENT LAW MOD FIVE: a full row carries moment 0+1+…+9 = 45 ≡ 0, so clears are INVISIBLE to the column moment mod five and any cycle must satisfy 5 ∣ Σ(4·col + shapeMoment) — the first cycle law constraining WHERE pieces are dropped, not just which are played |
| **`wordPolicy_eval`**, `wordOrbit`, `wordPolicy`, `wordPlay_mod`, `wordOrbit_nodup` | The realizing table policy: a legal 35-word's orbit has no repeated states, so the lookup policy "find the state in the orbit, play that letter" reproduces the word exactly at every orbit position |
| **`legal_word_draw_legal`**, `getD_drop_add` | Every draw of a legal word is LEGAL: at each step the played piece is present in the current bag — it is the next fresh piece of its block (block nodup), and the bag holds exactly the block's undrawn remainder (bag_stream_take_val) |
| **`bag_stream_take_val`** | The bag's EXACT VALUE along block play: after n draws from full, the bag is full minus precisely the first n mod 7 pieces of block n/7 — the set-valued sharpening of the card clock, en route to draw-legality of the realized policy |
| **`legal_cycle_thirty_five_distinct_states`**, `legal_cycle_orbit_distinct` | THE ORBIT IS A TRUE 35-CYCLE: a legal cycle word's first 35 states are pairwise distinct — the bag clock separates mod 7, the mass clock mod 5, coprime. The M2 object cannot degenerate |
| **`legal_cycle_orbit_signature`**, `wordPlay_bag_card`, `wordPlay_count_mod`, `map_piece_blocks` | THE ORBIT SIGNATURE: along a legal play's first period every state wears two clocks — bag card 7 − n mod 7 and board count b.count + 4n mod 10 — so states at different times coincide only when times agree mod 35 |
| **`bag_stream_take_card`** | The bag-card signature of block-structured play: after n draws from full along full seven-blocks, the bag holds EXACTLY 7 − n mod 7 pieces — full at every block boundary, draining one per draw between |
| **`foldl_draw_prefix`**, `foldl_draw_prefix_card` | Mid-block draws are exact set differences: drawing distinct present pieces, strictly fewer than the bag holds, removes exactly those pieces (the refill guard never fires) — the bag loses exactly one per draw |
| **`word_clear_mix_bound`**, `legal_cycle_word_tetris_free_moves` | The dichotomy folded: rows cleared ≤ 3·(clearing moves) + tetrises; a tetris-free legal cycle must harvest on ≥ 2n/15 of its moves — a tetris-free 35-cycle clears on at least five moves |
| **`move_clear_dichotomy`**, `clears_le_row_span`, `non_I_row_span_le_three`, `non_I_move_clears_le_three` | THE CLEAR DICHOTOMY: on a clear-free board every move clears ≤ 3 rows OR clears exactly four as the I — clears fit the piece's row span (every cleared row holds a fresh cell), and only the I spans four |
| **`legal_cycle_word_tetris_cap`**, `wordTetrises`, `wordTetrises_le_I_count`, `census_eq_count` | THE TETRIS CAP for words: every four-clear move is an I-move (via tetris_requires_I), the index census equals the value count, so a bag-legal cycle word plays at most length/7 tetrises — one per bag, the bag's single I |
| **`legal_cycle_word_survives_forever`**, `wordPlay` (+`_eq_stepWord_take`, `_periodic`) | THE INFINITE MILL: a bag-legal cycle word that never tops out within its first period, repeated forever, NEVER tops out — periodicity reduces every future state to a first-period state. A safe legal cycle word is a complete proof of infinite play: M2 ⟹ M1 at the level of pure words |
| **`legal_cycle_word_state_cycle`**, `stepWord` (+`_board`, `_bag`), `bag_stream_list_foldl`, `legal_word_bag_reset` | THE FULL-STATE LEGAL CYCLE: a bag-legal cycle word played as game-state steps from ⟨b, full bag⟩ returns the ENTIRE state — board rides its 35-quantum orbit, bag beats its seven-pulse, every draw legal. The shape of the M2 object; all that remains open is exhibiting one such word |
| **`bag_refills_after_full_block`**, `foldl_draw_exhaust`, `full_block_nodup` | The bag block reset: drawing exactly the pieces a bag holds — each once, any order — hits the refill guard on the last draw and ends at the FULL bag; a seven-piece all-present block has no repeats, so bag legality makes the bag component periodic with period seven |
| **`legal_cycle_ledger`** | THE LEGAL CYCLE LEDGER capstone: for any bag-legal repeatable cycle word — length a positive multiple of 35, each piece exactly n/7 times, exactly 2n/5 rows cleared, and clearing moves between a tenth and two fifths of the word. The M2 witness must run on these clocks |
| **`legal_cycle_word_clearing_moves_bracket`**, `wordClearMoves`, `wordClearMoves_le_wordClears`, `wordClears_le_four_mul_moves` | THE CLEARING-MOVES BRACKET for words: harvests reap 1–4 rows each (the ≤4 needs only clear-freeness, which cycling supplies), so a legal 35-cycle clears on between 4 and 14 of its moves — the trace-level bracket recovered for pure words |
| **`five_O_word_clears_two`**, `ten_I_word_clears_four`, `wordClears_append` | The mills' harvests from arithmetic alone: the census already knows the five-O word clears exactly two rows and the ten-I word exactly four (the tetris) — no board computation; clears add along concatenation |
| **`cycle_word_clear_census`**, `wordClears`, `foldl_count_ledger_exact`, `legal_cycle_word_clears_fourteen` | THE CLEAR CENSUS: any valid cycle word clears EXACTLY 2/5 of a row per move (5·clears = 2·length, with the clear total named, not existential) — a legal 35-word clears exactly FOURTEEN rows per period, the trace-level law recovered for pure placement words |
| **`legal_cycle_word_piece_census`**, `bag_word_piece_census`, `wordStream_eq_of_lt` | THE LEGAL CYCLE'S PIECE CENSUS: any bag-legal repeatable cycle word contains each of the seven pieces exactly length/7 times — a 35-word deals each piece exactly five times; the mill structure (five services per period) is forced by arithmetic alone |
| **`legal_cycle_word_thirty_five_dvd`**, `legal_cycle_word_min_length`, `wordStream` (+`_periodic`) | THE 35-QUANTUM FOR LEGAL CYCLES: a word folding a WF board to itself whose repetition deals full bags has length divisible by 35 (mass clock × bag heartbeat, coprime) — so legal cycles have ≥ 35 moves and the five-O / ten-I mills are PROVABLY illegal under the seven-bag |
| **`bag_stream_period_seven_dvd`**, `IsBagStream`, `bag_block_count`, `bag_stream_range_count`, `periodic_count_mul`, `periodic_add_mul`, `piece_univ_card` | THE BAG'S HEARTBEAT CANNOT BE COMPRESSED: any periodic piece stream dealing full bags has period divisible by seven — each block deals each piece exactly once (pigeonhole), so counting one piece over seven periods gives n = 7c |
| **`five_O_ritual_boards_on_cycle`** | The plinth tour: EVERY intermediate board of the five-O ritual (bands of width 0, 2, 4, 6, 8) sits on a closed five-cycle by riding the rotated word — the ritual's whole orbit is cyclic, not just its level anchor |
| **`board_on_cycle_shift`**, `BoardOnCycle.iterate`, `two_col_band_on_cycle` | Cycles rotate (every mid-cycle board is itself on a closed cycle) and iterate; the two-column plinth {0,1}×{rows 0,1} rides the rotated five-O word — closed cycles are NOT confined to the level stratum |
| **`clearLines_no_fullRows`**, `applyStep_clear_free`, `board_on_cycle_clear_free`, `clearedBelow_shift_strictMono`, `clearedBelow_le` | Every post-move board is clear-free: gravity keeps surviving rows strictly ordered, so a full row after clearing would force a full source row — hence CYCLING BOARDS ARE CLEAR-FREE: the mills' hypothesis is necessary, not just convenient |
| **`level_board_cycle_lengths`**, `board_cycle_length_quantum`, `foldl_applyStep_count_ledger`, `foldl_applyStep_wf`, `BoardOnCycle.add`, `empty_board_cycle_lengths` | THE COMPLETE CYCLE-LENGTH SPECTRUM: closed board cycles exist in EXACTLY the positive multiples of five on every clear-free level board (and on ∅) — existence by iterating the five-O mill, exclusion by the folded mass ledger (10C = 4n); `BoardOnCycle` upgraded to valid words |
| **`level_board_on_five_cycle`**, `level_board_on_ten_cycle`, `BoardOnCycle`, `empty_board_on_two_cycles`, `five_O_cycle_of_level`, `ten_I_cycle_of_level`, `low_of_level_wf` | Cycles are EVERYWHERE on the level stratum: for well-formed boards, clear-free + level suffices (lowness is free via lt_colHeight), and every such board sits on both a five-cycle and a ten-cycle, packaged as fold-words via `BoardOnCycle` |
| **`ten_I_cycle`**, `ten_I_cycle_empty` | THE TEN-I CYCLE: from any clear-free all-level board, ten vertical I's (one per column) build the four-row band and clear a TETRIS back to the EXACT starting board — the five-O cycle's tall sibling; the empty board sits on (at least) two distinct closed cycles |
| **`clearLines_band_reset`**, `I_column_dropped_eq`, `column_band_extend`, `no_full_of_partial_band_Ico`, `ten_I_dry_step`, `ten_I_final_step`, `I_r1_shape_eq` | The ten-I ritual's machinery: the general k-row band reset (any full band above a clear-free board vanishes without a trace), the vertical I's drop as the exact column tower, and the dry/final steps — nine I's build the four-row band column by column, the tenth completes it and the QUADRUPLE clears back to the original board |
| **`fiveOPolicy_state_cycle_init`**, `fiveOPolicy_orbit_five_states`, `fiveOPolicy_trace_bag_init`, `bag_draw_absent_fixed`, `fiveOPolicy_piece` | THE WALTZ'S STATE CYCLE: from step one on, the FULL game state (board AND bag) repeats with period five — the bag freezes at `full.erase O` after one draw, and the entire orbit is five states, reachable from `init` in one move: the free-piece world's M2 object, explicit |
| **`fiveOPolicy_survives`**, `fiveOPolicy` (+`_eval`), `fiveOPolicy_trace_board`, `fiveOPolicy_board_period`, `fiveOPolicy_survives_init` | THE FIVE-O WALTZ: a closed-form five-line policy (read the band's marker cells, play the next square) whose ENTIRE trace has a closed-form orbit (base board + band of width 2·(n mod 5)), board-periodic with period five, and never tops out — from any clear-free level state with two rows of ceiling, and from `init` itself (free-piece world) |
| **`five_O_loop_safe`**, `five_O_state_board_cycle`, `five_O_intermediate_boards` | The five-O loop lifted to game states: the board component returns exactly, and with two rows of ceiling (h + 2 ≤ 20) every state along the ritual is non-lost — the cycle is closed AND safe all the way round |
| **`five_O_cycle_empty`** | The empty board rides a five-move loop: five squares on the even pairs take ∅ back to ∅ — the simplest closed cycle in all of Tetris, fully verified |
| **`five_O_cycle`**, `five_O_dry_step`, `five_O_final_step` | THE FIVE-O CYCLE: from any clear-free level board, five squares on the even pairs return the board to EXACTLY its starting state — a concrete closed board cycle, the even-tiling tour's minimal service loop as five explicit placements |
| **`band_extend`**, `no_full_of_partial_band`, `colHeight_union_high_cols` | the five-O intermediate-state bricks: partial bands never complete a row, leave higher columns untouched, and extend pairwise to the full band |
