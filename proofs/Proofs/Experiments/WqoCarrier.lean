...
/-!
# WQO / finite-basis route to `TetrisSolvableValid` — the monotonicity foundation

The empirical `monotone` probe established (32.4M checks, 0 non-clearing violations) that
`applyStep` preserves column-height domination *except across line clears*. This file
proves that fact as a structural lemma about the no-clear drop `Placement.place`:

  `domLE b β  →  domLE (place b pl) (place β pl)`

where `domLE b β := ∀ j, colHeight b j ≤ colHeight β j` (Dickson's wqo on `ℕ¹⁰`). This is
the keystone the finite-basis carrier rests on: closure checked at the *maximal* basis
surfaces then covers every dominated board below them. Line clears are handled separately
(bag-boundary basis); this file is the no-clear core.
-/
