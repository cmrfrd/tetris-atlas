...
use crate::modules::{FiLM, FiLMConfig, Mlp, MlpConfig};
use crate::tensors::TetrisPieceOrientationLogitsTensor;
use crate::tensors::{TetrisPieceOrientationTensor, TetrisPieceTensor};
use crate::tetris::{
...
impl TetrisSimpleImitationPolicy {
    pub fn init(vb: &VarBuilder, cfg: &TetrisSimpleImitationPolicyConfig) -> Result<Self> {
        ...
        let board_encoder = cfg
            ...
            .iter()
            .enumerate()
            .map(|(i, cfg)| Mlp::init(&vb.pp(format!("board_encoder_{}", i)), cfg).unwrap())
            .collect();
        let piece_film = cfg
            .piece_film_config
            .iter()
