...
use crate::device;
use crate::fdtype;
use crate::grad_accum::GradientAccumulator;
use crate::modules::{Mlp, MlpConfig};
use crate::tensors::TetrisPieceTensor;
use crate::tetris::{TetrisBoard, TetrisPiece, TetrisPiecePlacement};
...
pub struct PolicyValueNetworkConfig {
    pub board_feature_encoder_config: MlpConfig,
    pub piece_embedding_dim: usize,
    pub trunk_mlp_config: MlpConfig,
    pub policy_head_config: MlpConfig,
    pub value_head_config: MlpConfig,
}
...
impl PolicyValueNetwork {
    pub fn init(vb: &VarBuilder, cfg: &PolicyValueNetworkConfig) -> Result<Self> {
        let board_feature_encoder = Mlp::init(
        ...
        let trunk_mlp = Mlp::init(&vb.pp("trunk_mlp"), &cfg.trunk_mlp_config)?;
        let policy_head = Mlp::init(&vb.pp("policy_head"), &cfg.policy_head_config)?;
        let value_head = Mlp::init(&vb.pp("value_head"), &cfg.value_head_config)?;
        Ok(Self {
...
pub fn train_value_function_policy(
    ...
    let policy_value_network_config = PolicyValueNetworkConfig {
        board_feature_encoder_config: MlpConfig {
    ...
    let mut games = crate::tetris::TetrisGameSet::new(NUM_GAMES);
    let n = games.len();
