...
#[derive(Debug, Clone)]
pub struct TetrisWorldModelTokenizerTransformerBlock {
...
#[derive(Debug, Clone)]
pub struct TetrisWorldModelTokenizerTransformerBlockConfig {
    pub dyn_tanh_config: DynamicTanhConfig,
    pub attn_config: CausalSelfAttentionConfig,
    pub mlp_config: MlpConfig,
}

impl TetrisWorldModelTokenizerTransformerBlock {
    fn init(
        vb: &VarBuilder,
        cfg: &TetrisWorldModelTokenizerTransformerBlockConfig,
    ) -> Result<TetrisWorldModelTokenizerTransformerBlock> {
        ...
        Ok(TetrisWorldModelTokenizerTransformerBlock {
            dyn_tanh_1,
            attn,
            dyn_tanh_2,
            mlp,
...
/// fuse(board_embedding, piece_embedding, orientation_embedding)
///
/// Token Types:
/// - Goal token: fuse(board_embedding, <goal_token>)
/// - Current token: fuse(board_embedding, <current_token>)
/// - Action token: fuse(board_embedding, piece_embedding, orientation_embedding)
#[derive(Debug, Clone)]
pub struct TetrisWorldModelTokenizer {
    ...
    blocks: Vec<TetrisWorldModelTokenizerTransformerBlock>,
...
pub struct TetrisWorldModelTokenizerConfig {
    pub board_embedding_dim: usize,
    pub piece_embedding_dim: usize,
    pub orientation_embedding_dim: usize,

    pub blocks_config: TetrisWorldModelTokenizerTransformerBlockConfig,
...
impl TetrisWorldModelTokenizer {
    pub fn init(
        ...
        let dyn_tanh = DynamicTanh::init(
            ...
            &DynamicTanhConfig {
                ...
                normalized_shape: cfg.blocks_config.attn_config.d_model,
            },
        )?;
        ...
        Ok(TetrisWorldModelTokenizer {
            ...
            num_residuals: cfg.num_residuals,
            blocks,
            dyn_tanh,
            token_encoder,
        })
    ...
    fn inner_forward(
        &self,
        tetris_boards_dist: &TetrisBoardsDistTensor,
        piece: &TetrisPieceTensor,
        orientation: &TetrisPieceOrientationTensor,
    ) -> Result<Tensor> {
        let board_embedding = self.soft_forward_board_embeddings(tetris_boards_dist)?;
        ...
        let (b_bo, _t_bo, d_bo) = board_embedding.dims3()?;
        let (b_pi, d_pi) = piece_embedding.dims2()?;
        let (b_or, d_or) = orientation_embedding.dims2()?;
        ensure!(
            b_bo == b_or && b_bo == b_pi,
            "Board, piece, and orientation embeddings must have the same batch size"
        );
        ensure!(
            d_bo == d_or && d_bo == d_pi,
            "Board, piece, and orientation embeddings must have the same dimension"
        );

        // ([B, D] + [B, D]) -> [B, D] -> [B, 1, D]
        let merged_piece_orientation_embedding =
            (piece_embedding + orientation_embedding)?.unsqueeze(1)?;

        let mut x = board_embedding.clone();
        for (i, block) in self.blocks.iter().enumerate() {
            if i < self.num_residuals {
                x = block
                    .forward(x, false)?
                    .broadcast_add(&merged_piece_orientation_embedding)?;
            } else {
    ...
    pub fn forward_goal_token(&self, goal_board: &TetrisBoardsDistTensor) -> Result<Tensor> {
        ...
        let piece_tensor =
            TetrisPieceTensor::from_pieces(&vec![TetrisPiece::NULL_PIECE; b], &device)?;
        let orientation_tensor = TetrisPieceOrientationTensor::from_orientations(
            &vec![TetrisPieceOrientation::NULL_ORIENTATION; b],
            device,
        )?;

        // [B, T, D]
        let x = self.inner_forward(goal_board, &piece_tensor, &orientation_tensor)?;
    ...
    pub fn forward_current_token(
        ...
        let x = self.inner_forward(current_board, current_piece, &orientation_tensor)?;
        // [B, T, D] -> [B, D, T] -> [B, D, 1] -> [B, D]
        let token_encoded = self
            .token_encoder
            .forward(&x.transpose(1, 2)?)?
    ...
    pub fn forward_token(
        ...
        // [B, T, D]
        let x = self.inner_forward(board, piece, orientation)?;
    ...
    pub fn forward_context(
        ...
        orientations: &Vec<TetrisPieceOrientationTensor>,
...
impl TetrisWorldModelBlock {
    fn init(vb: &VarBuilder, cfg: &TetrisWorldModelBlockConfig) -> Result<TetrisWorldModelBlock> {
        let dyn_tanh_1 = DynamicTanh::init(vb, &cfg.dyn_tanh_config)?;
        let attn = CausalSelfAttention::init(vb, &cfg.attn_config)?;
        let dyn_tanh_2 = DynamicTanh::init(vb, &cfg.dyn_tanh_config)?;
...
impl TetrisWorldModel {
    pub fn init(vb: &VarBuilder, cfg: &TetrisWorldModelConfig) -> Result<TetrisWorldModel> {
        ...
        let dyn_tanh = DynamicTanh::init(vb, &cfg.dyn_tanh_config)?;
...
mod tests {
    ...
    fn test_tetris_world_model_tokenizer() -> Result<()> {
        ...
        let cfg = TetrisWorldModelTokenizerConfig {
            board_embedding_dim: embedding_dim,
            piece_embedding_dim: embedding_dim,
            orientation_embedding_dim: embedding_dim,
    ...
    #[test]
    fn test_tetris_world_model_tokenizer_overfit() -> Result<()> {
        let device = Device::Cpu;
        let varmap = VarMap::new();
        let vb = VarBuilder::from_varmap(&varmap, DType::F32, &device);
        ...
        let data_gen = TetrisDatasetGenerator::new();

        let batch_size = 4;

        // Create a simple target tensor of ones to overfit to
        let target = Tensor::ones(&[batch_size, embedding_dim], DType::F32, vb.device())?;
        ...
        for _ in 0..8 {
            let datum = data_gen.gen_uniform_sampled_transition(
                (0..8).into(),
                batch_size,
                &device,
                &mut rand::rng(),
            )?;
            let forward_goal = tokenizer
                .forward_goal_token(&TetrisBoardsDistTensor::try_from(datum.result_board)?)?;
            let forward_current = tokenizer.forward_current_token(
                &TetrisBoardsDistTensor::try_from(datum.current_board.clone())?,
                &TetrisPieceTensor::from_pieces(&datum.piece, &device)?,
            )?;
            let forward_token = tokenizer.forward_token(
                &TetrisBoardsDistTensor::try_from(datum.current_board)?,
                &TetrisPieceTensor::from_pieces(&datum.piece, &device)?,
                &datum.orientation,
            )?;

            let loss_goal = forward_goal.sub(&target)?.abs()?.sum_all()?;
            let loss_current = forward_current.sub(&target)?.abs()?.sum_all()?;
            let loss_token = forward_token.sub(&target)?.abs()?.sum_all()?;
            let loss = loss_goal.add(&loss_current)?.add(&loss_token)?;
    ...
    fn test_tetris_world_model_forward() -> Result<()> {
        ...
        assert!(
            !orientation_logits_all
                .flatten_all()?
                .to_vec1::<f32>()?
                .iter()
                .any(|&x| x.is_nan()),
            "Orientation logits contain NaNs"
        );
