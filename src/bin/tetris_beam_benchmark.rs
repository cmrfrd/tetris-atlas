...
use tetris_atlas::tetris::{IsLost, TetrisPieceOrientation};
...
-> Option<f64> {
    let mut game = tetris_atlas::tetris::TetrisGame::new();
    ...
    while pieces_played < TARGET_PIECES {
        ...
        let ScoredState { first_action, .. } =
            search.search_first_action_with_state(BeamTetrisState::new(game), MAX_DEPTH)?;
